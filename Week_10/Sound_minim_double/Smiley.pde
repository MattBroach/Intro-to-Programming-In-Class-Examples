class Smiley
{
  ...
  Smiley(float _x, float _y, float _r, color _face, String filename)
  {
    ...
    r = _r;
    face = _face;
    laugh = minim.loadSnippet(filename);
  }
  ...
  //function to make smiley play laugh
  void hah()
  {
    //isPlaying() returns true if the smiley() is playing
    //use this to not restart laugh while it's still playing
    if(!laugh.isPlaying())
    {
      laugh.rewind();
      laugh.play();
